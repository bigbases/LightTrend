...
if [ ! -d "./logs/NoNorm/Autoformer" ]; then
  mkdir ./logs/NoNorm/Autoformer
fi

if [ ! -d "./logs/NoNorm/FEDformer" ]; then
  mkdir ./logs/NoNorm/FEDformer
fi

if [ ! -d "./logs/NoNorm/iTransformer" ]; then
  mkdir ./logs/NoNorm/iTransformer
fi
...
for model_name in Autoformer FEDformer iTransformer; do
  ...
  for pred_len in 96 192 336 720; do
    ...
    python -u run_longExp.py \
      ...
      --gpu $gpu \
      --use_norm $use_norm \
      --itr 1 >logs/NoNorm/$model_name/traf_$pred_len.log
    ...
    python -u run_longExp.py \
      ...
      --gpu $gpu \
      --use_norm $use_norm \
      --itr 1 >logs/NoNorm/$model_name/wea_$pred_len.log
